...
OP_RE = re.compile(
    ...
    r"(NormalizeOrigin|MirrorOrbit|Checkpoint)"
)
...
def main() -> None:
    ...
    for template_id, record in templates.items():
        ...
        _, selected_u, selected_acs, _, _ = utility.get(template_id, (0, 0, 0, 0, 0))
    ...
    write_tsv(
        family_path,
        ["edge_uses", "templates", "selected_u", "selected_acs10", "semantic_family"],
        family_rows,
    )
    write_tsv(
        subsequence_path,
    ...
    utility_rows = []
    for template_id, record in templates.items():
        edge_uses, selected_u, selected_acs, min_slack, max_slack = utility.get(
            template_id, (0, 0, 0, 0, 0)
        )
        utility_rows.append(
            (
                template_id,
...
|---:|---:|---:|---|
"""
    for uses, count, selected_u, _, family in family_rows[:20]:
